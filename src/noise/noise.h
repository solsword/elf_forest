...
// (center, center) with an exponent of strength. Center must be in (0, 1).
// Negative values are flipped first and flopped afterwards, so it's really a
// double sigmoid. Note that there's a slight discontinuity at 0 which is
// exaggerated if the strength is low. A strength of 0 is just a straight line;
// negative strengths pull values towards the given center while positive
