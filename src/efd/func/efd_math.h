...
// as numbers.
efd_node * efd_fn_add(efd_node * node) {
  size_t i, count;
  efd_int_t r_int = 0;
  efd_num_t r_num = 0;
  ...
  for (i = 0; i < count; ++i) {
    if (use_ints) {
      r_int += efd_as_i(efd_nth(node, i));
    } else {
      r_num += efd_as_n(efd_nth(node, i));
    }
  }
...
efd_node * efd_fn_mult(efd_node * node) {
  size_t i, count;
  efd_int_t r_int = 1;
  efd_num_t r_num = 1;
  int use_ints = efd_return_type_of(node) == EFD_NT_INTEGER;
...
efd_node * efd_fn_div(efd_node * node) {
  efd_int_t i_num, i_den;
  efd_num_t n_num, n_den;
  if (efd_return_type_of(node) == EFD_NT_INTEGER) {
    i_num = efd_as_i(efd_nth(node, 0));
    i_den = efd_as_i(efd_nth(node, 1));
    return construct_efd_int_node(node->h.name, i_num / i_den);
  } else {
    efd_assert_return_type(node, EFD_NT_NUMBER);
    n_num = efd_as_n(efd_nth(node, 0));
    n_den = efd_as_n(efd_nth(node, 1));
...
efd_node * efd_fn_scale(efd_node * node) {
  efd_num_t input, max, min, result;

  efd_assert_return_type(node, EFD_NT_NUMBER);

  input = *efd_n(efd_nth(node, 0));
...
efd_node * efd_fn_weight(efd_node * node) {
  efd_node *wnode;
  efd_num_t *weights;
  size_t count, i;
  ...
  wnode = efd_nth(node, 0);
  count = efd__ai_count(weights);
  weights = *efd__ai(wnode);

  for (i = 0; i < count; ++i) {
    numerator += weights[i] * (*efd__n(efd_nth(node, i+1)));
